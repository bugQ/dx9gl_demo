...
bool eae6320::cTextureBuilder::Build( const std::vector<std::string>& )
{
	...
	D3DXIMAGE_INFO imageInfo_source;
	{
		...
		{
			wereThereErrors = true;
			UserOutput::Print( "DirectX failed to get image info for the source image", m_path_source );
			goto OnExit;
		}
	}
		...
		default:
			{
				...
				UserOutput::Print( errorMessage.str().c_str(), m_path_source );
				goto OnExit;
			}
		}
	}
	// Save the texture
	{
		...
		{
			wereThereErrors = true;
			UserOutput::Print( "DirectX failed to save the texture", m_path_target );
			goto OnExit;
		}
	}
...
namespace
{
	bool Initialize( const char* i_path_source )
	{
		// Create the D3D9 interface
		{
			...
			{
				eae6320::UserOutput::Print( "DirectX failed to create a Direct3D9 interface", i_path_source );
				return false;
			}
		}
		// Create the D3D9 device interface
		{
			...
			if ( FAILED( result ) )
			{
				...
				if ( FAILED( result ) )
				{
					eae6320::UserOutput::Print( "DirectX failed to create a Direct3D9 device", i_path_source );
					return false;
				}
			}
		}
